...
class Product extends CI_Controller
{
    ...
    public function list_files()
    {
        ...
        $arr = [];
        $mysql = $this->db->select('*')->from('mockupimage')->where('member_id',$this->session->userdata('member_login'))->get()->result();
...
//		$files = get_filenames($this->upload_path);
    ...
    public function edit_list_files()
    {
        $this->load->helper("file");
        $arr = [];
        $mysql = $this->db->select('*')->from('product_img')->where('product_id',$_GET['product_id'])->get()->result();
        foreach ($mysql as $item) {
...
//		$files = get_filenames($this->upload_path);
    ...
    {
        $nameimg = $_POST['nameimg'];
        $this->db->insert('product_img', array('product_img' => $nameimg, 'product_id'=>$_GET['product_id'],
            'member_id' => $this->session->userdata('member_login')));
    }
