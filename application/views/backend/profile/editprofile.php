<input type="hidden" id="member_id" value="<?= $this->session->userdata('member_login') ?>">
<?php $member = $this->AQueryView->get_membersedit($this->session->userdata('member_login'));
...
<script>
    ...
    function update_member() {
        if (confirm('ยืนยันการบันทึกการแก้ไขนี้หรือไม่')) {
            $.post("<?=site_url('BUpdateData/update_profile')?>", {
                ...
                location.href = "<?=site_url('Backend/editprofile')?>";
            });
        }
    }
</script>