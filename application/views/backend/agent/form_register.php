    ...
    <div class="col-md-12">
        <div class="card mb-4">
            ...
            <div class="row" style="padding: 20px">
                <div class="col-md-6">
                    <input type="hidden" id="adviser_id" value="<?= $this->session->userdata('member_login') ?>">
                    <div class="form-group">
                        <label>รหัสผู้แนะนำ</label>
                        <input type="text" class="form-control" id="adviser_code" readonly>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <button class="btn btn-nonbackground" id="btn-checkidcard" style="margin-top: 1%"
                                        onclick="valid_citizen_id();">ตรวจสอบ
                                </button>
                            </div>
                        </div>
                        <div class="col-md-6">
                ...
                <div class="col-md-12">
                    <button class="btn btn-background" style="float: right" onclick="confirmregister();">สมัครสมาชิก
                    </button>
                </div>
            </div>
        </div>
    ...
    $(document).ready(function () {
        ...
        $.getJSON("<?=site_url('Api/get_province')?>", function (data) {
            $.each(data, function (k, v) {
                $('#province').append('<option value="' + v['province_id'] + '">' + v['name'] + '</option>');
            })
        });
        $.getJSON("<?=site_url('Api/get_adviser?adviser_id=')?>" + $('#adviser_id').val(), function (data) {
            $.each(data, function (k, v) {
                $('#adviser_code').val(v['member_code']);
    ...
    function valid_citizen_id() {
        ...
        $.getJSON("<?=site_url('Api/check_idcard?idcard=')?>" + $('#idcard').val(), function (data) {
            if (data == true) {
                ...
                $('#txt-idcardfalse').html('เลขบัตรประชาชนไม่ถุกต้อง');
            }
        });
    }
        ...
        if ($('#username').val() == '' || $('#idcard').val() == '' || $('#telephone').val() == '' || $('#address').val() == ''
            ...
            if ($('#status_idcard').val() == 'true') {
                if (confirm('ยืนยันการสมัครสมาชิกหรือไม่')) {
                    $.post("<?=site_url('BInsertData/register_agent')?>", {
                        sex: $('#sex').val(),
                        ...
                        location.href = "<?=site_url('Backend/confirmprofile')?>"
                    });
                }
            } else {
