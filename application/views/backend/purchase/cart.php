                    ...
                    <div class="col-md-12">
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tbody>
                                <?php $orderline = $this->BQueryView->get_orderlinemockup();
                                $sumprice = 0.00;
                                $sumpoint = 0.00;
                                $express = 0.00;
                                $sumordermore = 0.00;
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <h6 class="txt-margin-price">
                                                    ...
                                                    THB</h6></td>
                                            <td><i class="fa fa-trash fa-lg del-trash" style="color: red"
                ...
                <div class="row" style="padding: 20px">
                    <div class="table-responsive">
                        <table class="table">
                            <tr>
                                <td><h6>ราคา : </h6></td>
                                <td><h6><?= number_format($sumprice, 2) ?> THB</h6></td>
                                <td><h6>คะแนน : </h6></td>
                                <td><h6><?= number_format($sumpoint, 2) ?></h6></td>
                            </tr>
                            <tr>
                                <td><h6>ค่าจัดส่ง : </h6></td>
                                <td><h6><?= number_format($express, 2) ?> THB</h6></td>
                                <td><h6>ส่วนลด : </h6></td>
                                <td>
                                    <h6><?= number_format($this->BQueryView->check_amountdiscount($this->BQueryView->get_checkpoint()), 2) ?>
                                        THB</h6></td>
                            </tr>
                            <tr>
                                <td colspan="3" style="text-align: center"><h6>รวมราคา : </h6></td>
                                <td>
                                    <h6><?= number_format(($sumprice + $express) - $this->BQueryView->check_amountdiscount($this->BQueryView->get_checkpoint()), 2) ?>
                                        THB</h6></td>
                            </tr>
                        </table>
                    </div>
                ...
                <div class="row" style="padding: 20px">
                    ...
                    <div class="col-md-12" style="margin-top: 5%">
                        <div class="float-right">
                            <button class="btn btn-background" onclick="confirm_cart();"><i class="fa fa-save"></i> บันทึก
                            </button>
                            <button class="btn btn-nonbackground" onclick="canclecart();"><i class="fa fa-trash"></i> ยกเลิก
                            </button>
...
<input type="hidden" id="discount"
       value="<?= $this->BQueryView->check_amountdiscount($this->BQueryView->get_checkpoint()) ?>">
<input type="hidden" id="sumdiscountprice"
       value="<?= ($sumprice + $express) - $this->BQueryView->check_amountdiscount($this->BQueryView->get_checkpoint()) ?>">
...
//            console.log(product_id +" -------- "+total );
            $.post("<?=site_url('BUpdateData/update_cartmockup')?>", {
    ...
    function delete_ordercart(product_id) {
        if (confirm('ยืนยันการลบข้อมูลนี้หรือไม่')) {
            $.post("<?=site_url('BDeleteData/delete_cartmockupline')?>", {
                product_id: product_id
            }, function () {
                location.reload();
            });
    ...
    function update_total() {
        $.post("<?=site_url('BUpdateData/update_total_mockup')?>", {
    ...
    function deleteproduct(cartmockupline_id) {
        if (confirm('ยืนยันการยกเลิกสินค้านี้หรือไม่')) {
            $.post("<?=site_url('BDeleteData/deletedata_mockup')?>", {
    ...
    function canclecart() {
        if(confirm('ยืนยันการยกเลิกรายการนี้หรือไม่')){
            $.post("<?=site_url('BUpdateData/cancle_cartmcokup')?>",function () {
                location.href = "<?=site_url('Backend/index')?>";
            });
        }
    }
...
//        console.log($('input[name="optradio"]:checked').val());
        if ($('input[name="optradio"]:checked').val() == 'โอนเงิน') {
            ...
            if(confirm('ยืนยันการสั่งชื้อชื้อสินค้านี้หรือไม่')){
                $.post("<?=site_url('BInsertData/comfirm_cart')?>", {
                    // ข้อมูลที่อยู่ใหม่
                    ...
                    location.href = "<?=site_url('Backend/purchase_senderorder')?>";
                });
            }
    ...
    function confirm_payments() {
        $.post("<?=site_url('BInsertData/tranfer_sliper')?>", {
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <button class="btn btn-primary" onclick="update_total();" style="background-color: #273F89;
                border: 1px solid #273F89;"><i class="fa fa-save"></i> บันทึก
                </button>
            </div>
